...
if [ "$answer" != "${answer#[Yy]}" ]; then
  ...
  xmlstarlet sel -t -m "//fideid" -v . -n standard_$1frl_xml.xml > ids.txt
  xmlstarlet sel -t -m "//rating" -v . -n standard_$1frl_xml.xml > ratings.txt
  paste -d , ids.txt ratings.txt > pairs.csv
  python3 wd-triplets-quiet.py

  printf "Cleaning up.\n"
  rm pairs.csv ids.txt ratings.txt standard_$1frl_xml.zip standard_$1frl_xml.xml
